package com.example.data

import android.util.Log
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.collectLatest
import kotlinx.coroutines.flow.first
import kotlinx.coroutines.flow.firstOrNull
import kotlinx.coroutines.launch

class ServiceRepository(
    private val serviceDao: ServiceDao,
    private val firestoreService: FirestoreService
) {

    val allProviders: Flow<List<ServiceProvider>> = serviceDao.getAllProviders()
    val allBookings: Flow<List<Booking>> = serviceDao.getAllBookings()
    val allConversations: Flow<List<ChatConversation>> = serviceDao.getAllConversations()
    val allUsers: Flow<List<UserAccount>> = serviceDao.getAllUsers()

    fun getProviderById(id: Int): Flow<ServiceProvider?> = serviceDao.getProviderById(id)

    fun getBookingsForProvider(providerId: Int): Flow<List<Booking>> = serviceDao.getBookingsForProvider(providerId)

    fun getMessagesForProvider(providerId: Int): Flow<List<ChatMessage>> = serviceDao.getMessagesForProvider(providerId)

    suspend fun getUserByEmail(email: String): UserAccount? = serviceDao.getUserByEmail(email)

    suspend fun insertUser(user: UserAccount) {
        serviceDao.insertUser(user)
        firestoreService.saveUserAccount(user)
    }

    suspend fun insertProvider(provider: ServiceProvider) {
        serviceDao.insertProviders(listOf(provider))
        firestoreService.saveProvider(provider)
    }

    /**
     * Start background synchronization between Firestore and Room Local Cache.
     * When online, Firestore streams updates in real time, and we upsert them to Room.
     * This keeps our local offline-first Room-driven UI completely synchronized.
     */
    fun syncWithFirestore(scope: CoroutineScope) {
        scope.launch {
            firestoreService.isLiveConnection.collectLatest { isLive ->
                if (isLive) {
                    Log.d("ServiceRepository", "Firestore is online. Launching bidirectional sync tasks...")
                    
                    // 1. Sync Service Providers
                    launch {
                        firestoreService.streamProviders().collectLatest { remoteProviders ->
                            if (remoteProviders.isNotEmpty()) {
                                Log.d("ServiceRepository", "Syncing ${remoteProviders.size} providers from Firestore to Room")
                                serviceDao.insertProviders(remoteProviders)
                            }
                        }
                    }

                    // 2. Sync Bookings
                    launch {
                        firestoreService.streamBookings().collectLatest { remoteBookings ->
                            if (remoteBookings.isNotEmpty()) {
                                Log.d("ServiceRepository", "Syncing ${remoteBookings.size} bookings from Firestore to Room")
                                remoteBookings.forEach { booking ->
                                    serviceDao.insertBooking(booking)
                                }
                            }
                        }
                    }

                    // 3. Sync Conversations
                    launch {
                        firestoreService.streamConversations().collectLatest { remoteConversations ->
                            if (remoteConversations.isNotEmpty()) {
                                Log.d("ServiceRepository", "Syncing ${remoteConversations.size} conversations from Firestore to Room")
                                remoteConversations.forEach { conversation ->
                                    serviceDao.insertConversation(conversation)
                                }
                            }
                        }
                    }
                    
                    // 4. Sync Active Chat Messages (for existing conversations)
                    launch {
                        serviceDao.getAllConversations().collectLatest { localConversations ->
                            localConversations.forEach { conversation ->
                                launch {
                                    firestoreService.streamMessages(conversation.providerId).collectLatest { messages ->
                                        if (messages.isNotEmpty()) {
                                            messages.forEach { msg ->
                                                serviceDao.insertMessage(msg)
                                            }
                                        }
                                    }
                                }
                            }
                        }
                    }

                    // 5. Sync Users
                    launch {
                        firestoreService.streamUsers().collectLatest { remoteUsers ->
                            if (remoteUsers.isNotEmpty()) {
                                Log.d("ServiceRepository", "Syncing ${remoteUsers.size} user accounts from Firestore to Room")
                                remoteUsers.forEach { user ->
                                    serviceDao.insertUser(user)
                                }
                            }
                        }
                    }
                } else {
                    Log.d("ServiceRepository", "Firestore is offline/sandbox. Running on pure offline Room mode.")
                }
            }
        }
    }

    suspend fun createBooking(booking: Booking): Long {
        // Generate a positive unique ID for synchronized persistence
        val finalId = if (booking.id == 0) (System.currentTimeMillis() and 0x7FFFFFFF).toInt() else booking.id
        val bookingToSave = booking.copy(id = finalId)
        
        val localRowId = serviceDao.insertBooking(bookingToSave)
        
        // Push update to Firestore (write-through)
        firestoreService.saveBooking(bookingToSave)
        
        return localRowId
    }

    suspend fun updateBookingStatus(bookingId: Int, status: String) {
        serviceDao.updateBookingStatus(bookingId, status)
        firestoreService.updateBookingStatus(bookingId.toString(), status)
    }

    suspend fun rateBooking(bookingId: Int, providerId: Int, rating: Int, reviewText: String) {
        serviceDao.updateBookingRating(bookingId, rating, reviewText)
        firestoreService.updateBookingRating(bookingId.toString(), rating, reviewText)
        
        // Recalculate and update provider rating locally
        serviceDao.updateProviderRating(providerId, rating)
        
        // Push updated rating to Firestore
        val provider = serviceDao.getProviderById(providerId).first()
        if (provider != null) {
            firestoreService.updateProviderRating(providerId, provider.rating, provider.reviewCount)
        }
    }

    suspend fun sendChatMessage(providerId: Int, senderRole: String, messageText: String) {
        val uniqueId = (System.currentTimeMillis() and 0x7FFFFFFF).toInt()
        val message = ChatMessage(
            id = uniqueId,
            providerId = providerId,
            senderRole = senderRole,
            messageText = messageText,
            timestamp = System.currentTimeMillis()
        )
        serviceDao.insertMessage(message)
        firestoreService.saveChatMessage(message)

        // Find or build conversation
        val providersList = serviceDao.getAllProviders().first()
        val provider = providersList.find { it.id == providerId }
        val name = provider?.name ?: "Provider #$providerId"
        val category = provider?.category ?: "Service"
        val avatar = provider?.avatarUrl ?: ""

        val conversation = ChatConversation(
            providerId = providerId,
            providerName = name,
            providerCategory = category,
            providerAvatarUrl = avatar,
            lastMessageText = messageText,
            lastMessageTimestamp = System.currentTimeMillis(),
            unreadCount = if (senderRole == "PROVIDER") 1 else 0
        )
        serviceDao.insertConversation(conversation)
        firestoreService.saveConversation(conversation)
    }

    suspend fun seedDatabaseIfEmpty() {
        // Check if providers are empty in local room db
        val current = serviceDao.getAllProviders().first()
        val seedList = listOf(
            ServiceProvider(
                id = 1,
                name = "Alex Morgan",
                category = "Plumbing",
                rating = 4.8,
                reviewCount = 24,
                experience = "8 years",
                hourlyRate = 45.0,
                bio = "Professional plumbing services, leak fixes, pipe repairs, and emergency plumbing. 100% customer satisfaction guaranteed.",
                avatarUrl = "https://images.unsplash.com/photo-1540569014015-19a7be504e3a?w=150",
                availabilitySlots = "09:00 AM,11:00 AM,02:00 PM,04:00 PM",
                isFeatured = true
            ),
            ServiceProvider(
                id = 2,
                name = "Maya Patel",
                category = "Tutoring",
                rating = 4.9,
                reviewCount = 42,
                experience = "5 years",
                hourlyRate = 35.0,
                bio = "Experienced Calculus, Algebra, and SAT tutor. Certified math teacher with a passion for student success across all grade levels.",
                avatarUrl = "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=150",
                availabilitySlots = "10:00 AM,01:00 PM,03:00 PM,05:00 PM",
                isFeatured = true
            ),
            ServiceProvider(
                id = 3,
                name = "Clara Reynolds",
                category = "Cleaning",
                rating = 4.7,
                reviewCount = 18,
                experience = "3 years",
                hourlyRate = 25.0,
                bio = "Meticulous deep cleaning and regular housekeeping. I use eco-friendly products to give your apartment a spotless, healthy shine.",
                avatarUrl = "https://images.unsplash.com/photo-1567532939604-b6b5b0db2604?w=150",
                availabilitySlots = "08:00 AM,11:00 AM,01:00 PM,04:00 PM",
                isFeatured = false
            ),
            ServiceProvider(
                id = 4,
                name = "James Carter",
                category = "Electrical",
                rating = 4.9,
                reviewCount = 31,
                experience = "10 years",
                hourlyRate = 50.0,
                bio = "Licensed master electrician for residential and light commercial needs. Smart home setups, fixture installs, and diagnostic testing.",
                avatarUrl = "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150",
                availabilitySlots = "09:00 AM,11:30 AM,02:30 PM,04:30 PM",
                isFeatured = true
            ),
            ServiceProvider(
                id = 5,
                name = "Liam O'Connor",
                category = "Fitness",
                rating = 4.6,
                reviewCount = 12,
                experience = "4 years",
                hourlyRate = 40.0,
                bio = "Certified NASM personal trainer. Holistic health guidance, strength conditioning, weight-loss programming, and custom workout strategies.",
                avatarUrl = "https://images.unsplash.com/photo-1517841905240-472988babdf9?w=150",
                availabilitySlots = "10:00 AM,12:00 PM,03:00 PM,06:00 PM",
                isFeatured = false
            )
        )

        // Seed modern Room local backup if empty
        if (current.isEmpty()) {
            serviceDao.insertProviders(seedList)
        }

        // Also proactively upload to Firestore so their Firestore console is immediately useful on first boot
        firestoreService.isLiveConnection.first { isLive ->
            if (isLive) {
                Log.d("ServiceRepository", "Seeding initial service listings to Firebase Firestore...")
                seedList.forEach { provider ->
                    firestoreService.saveProvider(provider)
                }
            }
            true
        }
    }
}
